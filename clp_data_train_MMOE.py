...
from config import *
from utils import *
from model import BERT4NILM, MMOE
import argparse
import torch
# torch.conv2d()
...
def train(args, export_root=None, resume=True):
    ...
    val_loader = DataLoader(test_set, batch_size=args.batch_size, shuffle=True, num_workers=0,drop_last=True)

    trainer = Trainer_MMOE(args, model, train_loader,
                      val_loader, stats, export_root, train_set)
    ...
    # np.save('rr_record.npy',np.array(trainer.res_record))
