...
class REDD_LF_Dataset(AbstractDataset):
    ...
    def load_data(self):
        ...
        if not self._if_data_exists():
            ...
            # print('RAW_DATASET_ROOT_FOLDER',RAW_DATASET_ROOT_FOLDER)
    ...
    def load_data(self):
        ...
        if not self._if_data_exists():
            ...
            for house_id in self.house_indicies:
                ...
                if np.sum(list(app_index_dict.values())) == -len(self.appliance_names):
                    self.house_indicies.remove(house_id) # if no data for any appliance, remove house
                    continue
                ...
                if house_id == self.house_indicies[0]: # for the first house_id
                    entire_data = house_data
